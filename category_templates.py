...
CATEGORY_PROFILES = {
    ...
    "beauty": {
        ...
        "hook_pattern": "**반드시 의문문(?) 또는 충격 단어**로 시작 — 예: '이거 진짜 효과 있어?', '왜 다 이걸 사?', '한 번 쓰면 못 끊는 토너'",
...
def format_category_hint(category_id: str) -> str:
    ...
    강화 (2026-04-27):
      - Hook 단어/구두점 강제 (실측 결과: beauty 가이드가 hook 단어 사용을 억제했음)
      - 출력 길이 제한 (실측: 가이드가 +131자 길게 만들어 TTS 시간 초과 위험)
      - 마크다운/메타정보 금지 (편집 포인트 등 실제 영상에 안 들어가는 텍스트)
    """
    ...
    return (
        ...
        f"- 금지사항: {do_not_str}\n"
        f"\n[출력 규칙 — 반드시 지킬 것]\n"
        f"- 첫 문장(Hook)은 25자 이내, 의문문(?) 또는 강한 단어(진짜/충격/비밀/이거/왜) 중 하나 포함\n"
        f"- 전체 내레이션 글자 수 200자 이내 (30초 음성 분량)\n"
        f"- 마크다운(#, **, ---) 금지, 편집 메타정보(촬영팁/효과음 등) 금지\n"
        f"- 자막/내레이션 분리 표기하지 말고 자연스러운 한 편의 대본만 출력\n"
    )
